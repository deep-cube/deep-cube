...
const CubeSim = function (width: number, height: number, colorScheme?: Array<number>, mode?: string,
    faces?: Face[]) {
    ...
    //else
        camera.position.copy(new THREE.Vector3(0 / 1.1, 4 / 1.1, 4 / 1.1))
    ...
    function drawCube(faces: FaceletCubeT): THREE.Group {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            tmp.multiply(rotation_matrix).multiply(rot).multiply(rot1)
            //cubie.setRotationFromMatrix(rotation_matrix.multiply(rot.multiply(rot1)))
            cubie.setRotationFromMatrix(tmp)
            for (let z = -1; z <= 1; z++) {
                for (let x = -1; x <= 1; x++) {
                    let idx = (z + 1) * 3 + (x + 1)
                    ...
                    if (facesToReveal.indexOf(i) > -1) { // (i === 5 && mode === "UF")) {
                        ...
                        stickerhint.position.copy(new THREE.Vector3(x * 2, 3 + 3, z * 2))
    ...
    const updateRotation = (rotation: THREE.Matrix4) => {
...
let cubeSim = CubeSim(370, 370)

cubeSim.updateColorScheme(CubeUtil.ori_to_color_scheme("WG"))

function CubeSimView(props: {width: number, height: number, cube: FaceletCubeT, rotation?: THREE.Matrix4} ) {
    const mount = React.useRef<HTMLDivElement | null>(null)
    let { width, height, cube, rotation } = props

    React.useEffect(() => {
        if (rotation) {
            cubeSim.updateRotation(rotation)
            cubeSim.renderScene()
        }
    }, [rotation])

    React.useEffect(() => {
        cubeSim.updateWidthHeight(width, height, "#eeeeef")
        cubeSim.renderScene()
    }, [width, height])

    React.useEffect(() => {
        cubeSim.updateCube(cube)
        cubeSim.renderScene()
    }, [cube])
