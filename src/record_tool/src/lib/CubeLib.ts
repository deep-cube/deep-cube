...
let Cube = (function () {
    ...
    */
    ...
    let create = (arg : String | MoveT | Array<MoveT>) => {
        if (typeof arg === "string") {
            return from_move(Move.parse(arg))
        } else {
            const arg_ = arg as MoveT | Array<MoveT>
            return from_move(arg_)
        }
    }

    let to_facelet = (cube: CubieT) => {
        return FaceletCube.from_cubie(cube)
    }
...
let Move = function () {
    ...
    let from_moves = (moves: Array<MoveT>, name: string): MoveT => {
        let move = from_cube(Cube.apply(Cube.id, moves), name)
        return move
    }
...
let CubeUtil = (() => {
    ...
    let is_mask_solved = (cube: CubieT, { co, eo, cp, ep }: Mask, premove: (MoveT | MoveT[])[]) => {
        ...
        for (let move of premove) {
            let cube1 = Cube.apply(cube, move)
            let solved = true
    ...
    let get_random_lse = (): CubieT => {
        ...
        return Cube.apply(cube, rand_choice(m2_premove))
    }
...
export { Cube, Move, FaceletCube, CubeUtil, Mask }